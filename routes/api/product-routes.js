...
router.get("/:id", async (req, res) => {
  ...
  try {
    ...
    if (product_data) {
      ...
      res
        .status(404)
        .json({ message: "No product with the given ID was found!" });
    }
  } catch (err) {
    res.status(500).json(err);
  }
...
router.post("/", async (req, res) => {
  try {
    ...
    */
    ...
    const message =
      "Note: refactored code returns full reference to new product.";
...
router.put("/:id", async (req, res) => {
  try {
    ...
    const message =
      "Note: refactored code returns full reference to updated product.";
